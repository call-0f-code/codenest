...
export const ProfileHeader = ({
  ...
  return (
    <section className="relative bg-[#F5E6D3] dark:bg-[#2C1810] border-4 border-black p-7 shadow-[8px_8px_0_rgba(0,0,0,1)] flex flex-col md:flex-row items-center justify-between gap-8 rounded-none overflow-hidden hover:rotate-[0.5deg] hover:scale-[1.01] transition-transform duration-300 ease-in-out">
      {/* --- Left: Info --- */}
      <div className="flex-1 text-center md:text-left space-y-2 z-20">
        ...
        {user?.isManager && (
          <span className="inline-block mt-2 bg-[#C1502E] text-[#F5E6D3] border-4 border-black font-black px-3 py-0.5 shadow-[4px_4px_0_rgba(0,0,0,1)] text-sm rotate-[-1deg]">
            MANAGER
          </span>
        )}

        {/* Poster label */}
        <div className="absolute -top-1 left-4 bg-[#2C1810] text-[#F5E6D3] text-xs font-black px-3 py-0.5 border-4 border-black rotate-[-3deg] shadow-[3px_3px_0_rgba(0,0,0,1)]">
          PROFILE
        </div>
      </div>
      ...
      >
        {user.profilePhoto ? (
          ...
          <div className="flex items-center justify-center h-full text-[#F5E6D3]">
      ...
      <div className="absolute top-4 right-4 flex flex-wrap gap-3 z-30">
        {!isEditing ? (
          <button
            onClick={onEdit}
            className="bg-[#C1502E] text-[#F5E6D3] text-sm md:text-base font-black px-5 py-2 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] hover:-translate-y-[2px] transition-all flex items-center gap-1"
          >
          ...
          <>
            ...
            <button
              onClick={onCancel}
              className="bg-[#F5E6D3] text-[#2C1810] text-sm md:text-base font-black px-5 py-2 border-4 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] hover:-translate-y-[2px] transition-all flex items-center gap-1"
            >
